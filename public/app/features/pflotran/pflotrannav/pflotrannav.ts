...
import {DashboardExporter} from '../../dashboard/export/exporter';

export class PflotranNavCtrl {
  ...
  constructor($scope, $rootScope, alertSrv, $location, playlistSrv, backendSrv, $timeout, datasourceSrv) {
    ...
    $scope.zoomWorld = function() {   //tct
      $rootScope.$broadcast('zoom', {
        "zoomlevel": 4
      });
    };

    $scope.zoomSite = function() {
      $rootScope.$broadcast('zoom', {
        "zoomlevel": 16,
      });
    };
...
export function pflotranNavDirective() {
  return {
    restrict: 'E',
    templateUrl: 'public/app/features/pflotran/pflotrannav/pflotrannav.html',
    controller: PflotranNavCtrl,
    transclude: true,
  };
}

angular.module('grafana.directives').directive('pflotrannav', pflotranNavDirective);
